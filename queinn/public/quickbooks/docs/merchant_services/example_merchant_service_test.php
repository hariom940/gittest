...
$card_number = isset($_POST['credit-card-number']) && $_POST['credit-card-number']!='' ? $_POST['credit-card-number'] : '';
if($card_number!=''){
	$card_number = str_replace('+', '', $card_number);
	$card_number = str_replace(' ', '', $card_number);
}

$expDate = isset($_POST['credit-card-date']) && $_POST['credit-card-date']!='' ? $_POST['credit-card-date'] : '';
$expDate = explode('/',$expDate);
$cvv = isset($_POST['credit-validation-code']) && $_POST['credit-validation-code']!='' ? $_POST['credit-validation-code'] : '';
$address = isset($_POST['billing_address']) && $_POST['billing_address']!='' ? $_POST['billing_address'].', ' : '';
$address .= isset($_POST['billing_town']) && $_POST['billing_town']!='' ? $_POST['billing_town'] : '';
$postcode = isset($_POST['billing_postcode']) && $_POST['billing_postcode']!='' ? $_POST['billing_postcode'] : '';
...
if ($Transaction = $MS->authorize($Card, $amount))
{
	...
	{
		$_SESSION['error'] = 'An error occured during capture: ' . $MS->errorNumber() . ': ' . $MS->errorMessage();
		echo json_encode(array('error'=>array('An error occured during capture: ' . $MS->errorNumber() . ': ' . $MS->errorMessage() ), 'order_id'=>$order_id));
		exit;
	}
}
else
